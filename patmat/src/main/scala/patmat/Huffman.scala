...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def timesAcc(chars: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = {
      if (chars.isEmpty) acc
        ...
        val part = acc partition (x => x._1 == chars.head)
        val first = part._1
        if (first.isEmpty)
          timesAcc(chars.tail, acc ::: List((chars.head, 1)))
        else
          timesAcc(chars.tail, List((chars.head, first.head._2 + 1)) ::: part._2)
      }
    }
    timesAcc(chars, List())
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeAcc(subtree: CodeTree, bits: List[Bit], chars: List[Char]): List[Char] = subtree match {
      case Fork(left, right, _, _) => {
        require(bits.head == 0 || bits.head == 1)
        if (bits.head == 0) decodeAcc(left, bits.tail, chars)
      ...
      case Leaf(char, _) => {
        if (bits.isEmpty)
          ...
          decodeAcc(tree, bits, chars :+ char)
      }
    }
        ...
        subtree match {
          case Fork(left, right, _, _) => {
            if (chars(left) contains text.head)
              encodeAcc(left, text, bits :+ 0)
            else
              encodeAcc(right, text, bits :+ 1)
          }
          case Leaf(char, weight) => encodeAcc(tree, text.tail, bits)
        }
      }
    }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertAcc(subtree: CodeTree, table: CodeTable, bits: List[Bit]): CodeTable = subtree match {
      case Fork(left, right, _, _) => {
        mergeCodeTables(convertAcc(left, table, bits :+ 0),
          convertAcc(right, table, bits :+ 1))
      }
      case Leaf(char, _) => {
        table :+ (char, bits)
      }
